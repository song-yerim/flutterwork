/*
  Null safety타입 : 2.?? 버전부터 도입
    - 기본값 초기화 없이 선언하는 변수는 null 값에 대한 안전성을 보장하기 위해서
...
void main() {
  ...
  /*
      ! : nullable type으로 선언했을 때
          변수 뒤에 !를 붙여서 null인 경우 오류를 발생하도록 만듦
            - 변수명에 !를 붙이면, non-nullable의 타입이라는 뜻
  */

  String? name3 = 'won'; // null을 허용하는 타입
  print(name3);
  print(name3!); // null을 허용하지 않는 타입으로 변환됨
  ...
  // print(num1! + num2); // 오류 num1이 null값이기 때문에 연산 불가

  // ?? : null일 때 대체값 사용. 변수의 값은 변하지 않음
  print(num1 ?? 5);
  print(num1 ?? 7 + num2);
  // 대체적으로 괄호를 묶어 사용 => print((num1 ?? 7) + num2);
  print(num1);

  // if문을 null satefy를 이용하여 ?. 으로 사용할 수 있음
  String? name4 = "john";
  if(name4!= null) {
